/* global config, log, redisClient, doDonations, version */
  ...
  async.parallel({
    pool: function (callback) {
      redisClient.multi(redisCommands).exec(function (error, replies) {
        ...
        if (replies[5]) {
          for (var m in replies[5]) {
            if (config.poolServer.slushMining.enabled) {
              data.roundHashes += parseInt(replies[5][m]) / Math.pow(Math.E, ((data.lastBlockFound - dateNowSeconds) / config.poolServer.slushMining.weight)) // TODO: Abstract: If something different than lastBlockfound is used for scoreTime, this needs change.
            } else {
              data.roundHashes += parseInt(replies[5][m])
            }
          }
        }
    ...
    if (error) {
      ...
      zlib.deflateRaw(currentStats, function (error, result) { // eslint-disable-line
...
function broadcastLiveStats () {
  ...
  redisClient.multi(redisCommands).exec(function (error, replies) { // eslint-disable-line
...
function handleMinerStats (urlParts, response) {
  ...
  if (urlParts.query.longpoll === 'true') {
    redisClient.exists(config.coin + ':workers:' + address, function (error, result) { // eslint-disable-line
      if (!result) {
