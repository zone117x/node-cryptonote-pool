        ...
        function(balances, callback){
            ...
            async.filter(transferCommands, function(transferCmd, cback){
                apiInterfaces.rpcWallet('transfer', transferCmd, function(error, result){
                    ...
                    log('info', logSystem, 'Payments sent %j', [result]);
                    cback(false);
                });
            }, function(failedCmds){
                ...
                if (failedCmds.length > 0){
                    ...
                    fs.writeFile(failedFileName, JSON.stringify(failedCmds), function(err){
                        if (err){
                            log('error', logSystem, 'Failed to write payment failure data: %j', [failedCmds]);
                            callback(null);
                            return;
                        }
                        log('error', logSystem, 'Some payments failed; data logged to %s - must be executed manually', [failedFileName]);
                    });
                }
                log('info', logSystem, 'Payments splintered and %d sent, %d failed', [transferCommands.length - failedCmds.length, failedCmds.length]);
                redisClient.multi(redisCommands).exec(function(error, replies){
                    if (error){
                        log('error', logSystem, 'Super critical error with payments sending yet failing to be recorded in redis, double payouts likely to happen %j', [error]);
                        callback(true);
                        return;
                    }
                    callback(null);
