...
function runInterval () {
  async.waterfall([
    ...
    function (balances, callback) {
      ...
      for (var worker2 in payments) {
        var amount = parseInt(payments[worker2])
        transferCommands[commandIndex].rpc.destinations.push({amount: amount, address: worker2})
        transferCommands[commandIndex].redis.push(['hincrby', config.coin + ':workers:' + worker2, 'balance', -amount])
        transferCommands[commandIndex].redis.push(['hincrby', config.coin + ':workers:' + worker2, 'paid', amount])
