...
function cryptoNightFast (buf) {
  return cryptoNight(Buffer.concat([new Buffer([buf.length]), buf]), true) // eslint-disable-line
}
...
function BlockTemplate (template) {
  ...
  this.buffer = new Buffer(this.blob, 'hex') // eslint-disable-line
...
Miner.prototype = {
  ...
  getTargetHex: function () {
    ...
    var padded = new Buffer(32) // eslint-disable-line
    padded.fill(0)
    ...
    buffArray.reverse()
    var buffReversed = new Buffer(buffArray) // eslint-disable-line
...
function processShare (miner, job, blockTemplate, nonce, resultHash) {
  var shareBuffer = new Buffer(blockTemplate.buffer.length) // eslint-disable-line
  blockTemplate.buffer.copy(shareBuffer)
  shareBuffer.writeUInt32BE(job.extraNonce, blockTemplate.reserveOffset)
  new Buffer(nonce, 'hex').copy(shareBuffer, 39) // eslint-disable-line
  ...
  if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability) {
    hash = new Buffer(resultHash, 'hex') // eslint-disable-line
    shareType = 'trusted'
  } else {
  ...
  hashArray.reverse()
  var hashNum = bignum.fromBuffer(new Buffer(hashArray)) // eslint-disable-line
...
function startPoolServerTcp (callback) {
  async.each(config.poolServer.ports, function (portData, cback) {
    ...
    if (err) { callback(false) } else { callback(true) } // eslint-disable-line
  })
}
