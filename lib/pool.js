...
function Miner(id, login, pass, ip, startingDiff){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex){
    ...
    if (blockCandidate){
        redisCommands.push(['sadd', config.coin + ':blocksPending', [job.height, job.difficulty, hashHex, Date.now() / 1000 | 0].join(':')]);
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    var hash = cryptoNight(convertedBlob);
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        blockFastHash = cryptoNightFast(convertedBlob).toString('hex');
...
function handleMinerMethod(id, method, params, req, res, startingDiff){
    ...
    switch(method){
        case 'login':
            ...
            if (!utils.isValidAddress(params.login, config.poolAddress[0])){
            ...
            miner = new Miner(minerId, params.login, params.pass, req.connection.remoteAddress, startingDiff);
    ...
    async.each(config.poolServer.ports, function(portData, cback) {
        ...
        var getworkServer = http.createServer(function (req, res) {
            ...
            req.on('end', function () {
                ...
                handleMinerMethod(jsonData.id, jsonData.method, jsonData.params, req, res, portData.difficulty);
            });
        });
        ...
        getworkServer.listen(portData.port, function (error, result) {
            if (error) {
                log('error', 'Server', 'Could not start server listening on port ' + portData.port + ', error: ' + JSON.stringify(error));
                cback(true);
                return;
            }
            log('debug', 'Server', 'Started server listening on port ' + portData.port);
            cback();
        });
