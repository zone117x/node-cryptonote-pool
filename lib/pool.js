...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        var reversedShareBuffer = shareBuffer.toJSON();
        reversedShareBuffer.reverse();
        reversedShareBuffer = new Buffer(reversedShareBuffer);

        apiInterfaces.rpcDaemon('submitblock', [reversedShareBuffer.toString('hex')], function(error, result){
            if (error){
