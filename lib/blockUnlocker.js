...
function log(severity, message){
    logger[severity]('Unlocker', null, null, message);
}
    ...
    async.waterfall([
        ...
        function(blocks, callback){
            ...
            redisClient.multi(unlockedBlocksCommands).exec(function(error, replies){
                if (error){
                    log('error', 'Error with unlocking blocks ' + JSON.stringify(error));
                    callback(true);
                    return;
                }
                log('debug', 'Unlocked ' + totalBlocksUnlocked + ' blocks and update balances for ' + Object.keys(payments).length + ' workers');
                callback(null);
            });
